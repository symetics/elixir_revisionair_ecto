defmodule RevisionairEcto do
  ...
  def list_revisions(item_type, item_id, options) do
    ...
    |> Enum.map(fn {revision, inserted_at, {item, metadata}} ->
      put_in_metadata({decode_item(item), metadata}, %{revision: revision, inserted_at: inserted_at})
    end)
  end
  ...
  def newest_revision(item_type, item_id, options) do
    ...
    case repo.all(
      ...
      [{revision, inserted_at, {item, metadata}}] ->
        {:ok, put_in_metadata({decode_item(item), metadata}, %{revision: revision, inserted_at: inserted_at})}
    end
  end
